...
app.post('/signup',function(req,resp){
    ...
    var new_password = md5(md5(password).substr(1,88) + md5(password));
    ...
    db.register("Durhack", "users", registerInfo, function (err, result) {
            if(err){
                console.log(err);
                return;
            }
            if(result == 'the name has been used') {
                //返回到index
                resp.send([JSON.stringify("the name has been used")])
                return;
            }else if(result == '1'){
                //返回到index
...
app.get("/login", function(req,resp){
    ...
    db.show("Durhack", "users", username, function (err, result){
            if(err){
                console.log(err);
                return;
            }
            if(result[0].password == new_password) {
                ...
                //返回到index
                resp.send([JSON.stringify("login successful!")])
                return;
            }else if(result[0].password != new_password){
                console.log("error password!");
                //返回到index
                resp.send([JSON.stringify("error password!")]);
                return;
            }
    });
